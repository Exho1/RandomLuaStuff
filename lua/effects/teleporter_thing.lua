...
local mat_rising = Material( "models/wireframe")
...
function EFFECT:Init(data)
   ...
   self.BeamDownTime = CurTime() + 0.5
   self.EndTime = self.BeamDownTime + data:GetRadius()

   self.BeamDown = false

   if IsValid(self.Owner) then
      self.Dummy = ClientsideModel(self.Owner:GetModel(), RENDERGROUP_TRANSLUCENT)
